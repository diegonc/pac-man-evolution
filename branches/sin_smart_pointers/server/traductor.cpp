...
MapaBajoNivel* Traductor::traducir_mapa (Mapa* mapa){
	...
	for (int cont1 = 0; cont1 < mapa->get_alto(); cont1++)
		for (int cont2 = 0; cont2 < mapa->get_ancho(); cont2++){
			...
			EstructuralUnitario *  estruct_bajo_nivel;
...
void Traductor::conectar_casilleros(S_ptr<Casillero> origen, S_ptr<Casillero> destino, Mapa* mapa_alto_nivel, MapaBajoNivel* mapa_bajo_nivel, Orientacion orientacion){
		...
		if ((!origen.es_nulo()) && (!destino.es_nulo())){
			...
			if ((!estruct_destino.es_nulo()) && (estruct_destino->es_habitable(destino->get_pos_x(), destino->get_pos_y()))){
				...
				EstructuralUnitario *  est_bajo_nivel_origen = mapa_bajo_nivel->get_estructural(pos_origen);
				Posicion pos_destino(destino->get_pos_y(), destino->get_pos_x());
				EstructuralUnitario *  est_bajo_nivel_destino = mapa_bajo_nivel->get_estructural(pos_destino);
