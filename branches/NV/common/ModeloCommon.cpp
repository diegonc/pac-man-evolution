...
void ModeloCommon::set_posicion_inicial( Jugador * jugador ){
      ...
      //si es pacman
   	if (jugador->get_personaje()->get_tipo() == Personaje::pacman){
         ...
         std::list< EstructuralUnitario* > lista_CasaFantasma=get_mundo().get_mapa_activo()->get_casa_fantasma();	
		   //selecciono una posicion
         pos=pos % lista_CasaFantasma.size();
         //busco el estructural
		   std::list< EstructuralUnitario * >::iterator it = lista_CasaFantasma.begin();
      	for (int i=0 ; i<pos ; ++i)
			   ++it;
		   //lo elijo y le asigno esa posicion		   
           EstructuralUnitario * estructural_elegido = (*it);
