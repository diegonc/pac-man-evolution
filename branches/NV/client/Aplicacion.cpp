...
//			std::cout << "Aca se deberia enviar el mensaje: " << MensajeAEnviar  << std::endl;
			...
			Paquete* PaqMens=new PaqueteMensaje(MensajeAEnviar);
		...
		if(Keys[SDLK_UP]){
            ...
            Paquete  * PaqTec = new PaqueteTecla(JugadorLocal::get_instancia()->get_id(),0);
        ...
        if(Keys[SDLK_DOWN]){
            ...
            Paquete * PaqTec = new PaqueteTecla(JugadorLocal::get_instancia()->get_id(),2);
            cliente->get_escritor().encolar_paquete(PaqTec);
		    //kop.ejecutar(*(getAplicacionGrafica()->getModelo()));
            //bajo el flag
            Keys[SDLK_DOWN]=0;
        ...
        if(Keys[SDLK_LEFT]){
            ...
            Paquete  * PaqTec = new PaqueteTecla(JugadorLocal::get_instancia()->get_id(),3);
            cliente->get_escritor().encolar_paquete(PaqTec);
            Keys[SDLK_LEFT]=0;
        ...
        //lo mismo si derecha
        if(Keys[SDLK_RIGHT]){
            Paquete * PaqTec = new PaqueteTecla(JugadorLocal::get_instancia()->get_id(),1);
            cliente->get_escritor().encolar_paquete(PaqTec);
            Keys[SDLK_RIGHT]=0;
        }
	}
