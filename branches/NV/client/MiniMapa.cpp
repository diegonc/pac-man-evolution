...
void MiniMapa::DibujarComestible(SDL_Surface* Screen, Comestible * Comest, int ImgXCurr, int ImgYCurr){
...
void MiniMapa::Dibujar(Jugador* jug, SDL_Surface* Screen){
	...
	std::list< Comestible * > comestibles=ModeloServidor::get_instancia()->get_mundo().get_mapa_activo()->get_comestibles();
	...
	for(int i=0;i< (DimensionTablero * DimensionTablero);++i){
		...
		for (std::list< Comestible* >::iterator it=comestibles.begin();it!=comestibles.end();++it){
			Comestible * comest=*it;
...
bool MiniMapa::hayEstructural(Posicion& P){
	...
	//si la posicion es valida
		...
		EstructuralUnitario * estruc=mapa->get_estructural(P);
		if (estruc != NULL){
			return true;
		}else{
			return false;
